resource "kubernetes_namespace_v1" "kured" {
  ...
  count      = var.enable_auto_os_updates && local.is_highly_available ? 1 : 0
  metadata {
    name = "kured"
  }
...
data "http" "system_upgrade_controller_crds" {
  count = var.enable_auto_kubernetes_updates && local.is_highly_available && var.allow_remote_manifest_downloads ? 1 : 0
...
resource "kubectl_manifest" "system_upgrade_controller_crds" {
  depends_on = [terraform_data.wait_for_infrastructure]
  for_each   = var.enable_auto_kubernetes_updates && local.is_highly_available && var.allow_remote_manifest_downloads ? { for i in local.system_upgrade_controller_crds : index(local.system_upgrade_controller_crds, i) => i } : {}
...
resource "kubectl_manifest" "system_upgrade_controller_ns" {
  ...
  for_each   = var.enable_auto_kubernetes_updates && local.is_highly_available && var.allow_remote_manifest_downloads ? { for i in local.system_upgrade_controller_components : index(local.system_upgrade_controller_components, i) => i if strcontains(i, "kind: Namespace") } : {}
  yaml_body  = each.value
}
...
resource "kubectl_manifest" "system_upgrade_controller_server_plan" {
  ...
  count      = var.enable_auto_kubernetes_updates && local.is_highly_available ? 1 : 0
